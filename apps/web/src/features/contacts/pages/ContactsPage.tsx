...
export function ContactsPage() {
  ...
  const [lastDeleted, setLastDeleted] = useState<Contact | null>(null);
  ...
  const confirmDelete = () => mutations.remove.mutate(deleteTargets.map(({ id, version }) => ({ id, version })), { onSuccess: (deleted) => { setLastDeleted(deleted.length === 1 ? deleted[0] ?? null : null); setSelected(new Set()); setDeleteTargets([]); setDrawerId(null); notify({ title: 'Contacts deleted', tone: 'success' }); } });
  ...
  return <div className="grid gap-6">
    ...
    {lastDeleted ? <div className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm dark:border-amber-900 dark:bg-amber-950/40"><span>{lastDeleted.firstName} {lastDeleted.lastName} was deleted.</span><Button loading={mutations.restore.isPending} onClick={() => mutations.restore.mutate(lastDeleted, { onSuccess: () => { setLastDeleted(null); notify({ title: 'Contact restored', tone: 'success' }); } })} size="sm" variant="outline">Restore</Button></div> : null}
