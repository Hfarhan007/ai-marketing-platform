...
export class CompaniesController {
  ...
  @Get(':id') @RequirePermissions('companies.read') get(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
  ) {
    return this.s.get(c, id);
  }
  @Post() @RequirePermissions('companies.create') create(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Body() d: CreateCompanyDto,
  ) {
    return this.s.create(c, d);
  }
  @Patch(':id') @RequirePermissions('companies.update') update(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
    @Body() d: UpdateCompanyDto,
  ) {
    return this.s.update(c, id, d);
  }
  @Delete(':id') @RequirePermissions('companies.delete') remove(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
    @Query() d: VersionDto,
  ) {
  ...
  @Post(':id/restore') @RequirePermissions('companies.update') restore(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
    @Body() d: VersionDto,
  ) {
  ...
  @Post('jobs/import') @RequirePermissions('companies.create') imp(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Body() d: DataJobDto,
  ) {
    return this.s.createJob('import', c, d.options);
  }
  @Post('jobs/export') @RequirePermissions('companies.read') exp(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Body() d: DataJobDto,
  ) {
    return this.s.createJob('export', c, d.options);
  }
  @Post('bulk') @RequirePermissions('companies.update') bulk(
    @WorkspaceContext() c: WorkspaceRequestContext,
