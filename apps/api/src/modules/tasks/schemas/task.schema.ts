...
export class Task implements CrmEntity {
  ...
  @Prop({ type: String, enum: ['open', 'in_progress', 'completed', 'cancelled'], default: 'open' })
  status!: string;
  ...
  @Prop({ type: [MongooseSchema.Types.Mixed], default: [] }) checklist!: Record<
    string,
    string | boolean
  >[];
  ...
  @Prop({ type: MongooseSchema.Types.Mixed, default: null }) recurrence!: Record<
    string,
    string | number
  > | null;
  @Prop({ type: [MongooseSchema.Types.Mixed], default: [] }) completionHistory!: Record<
    string,
    Date | string
  >[];
