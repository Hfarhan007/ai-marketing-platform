...
export class PipelineStage {
  ...
  @Prop({ type: MongooseSchema.Types.Mixed, default: {} }) rules!: Record<string, string | number | boolean>;
}
...
export class Pipeline implements CrmEntity {
  ...
  @Prop({ type: String, required: true }) name!: string;
  @Prop({ type: [PipelineStageSchema], validate: [(v: PipelineStage[]) => v.length > 0, 'At least one stage is required'] }) stages!: PipelineStage[];
...
PipelineSchema.index({ workspaceId: 1, isDefault: 1 }, { unique: true, partialFilterExpression: { isDefault: true, deletedAt: null } });
