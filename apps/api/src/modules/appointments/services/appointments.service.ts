...
export class AppointmentsService {
  constructor(
    ...
    private readonly jobs: SchedulingJobsService,
  ...
  async list(c: WorkspaceRequestContext, q: CrmListQueryDto) {
    ...
    return { ...p, items: p.items.map(map) };
  }
  async get(c: WorkspaceRequestContext, id: string) {
    return map(await this.repository.getActive(c.workspaceId, id));
  }
  async create(c: WorkspaceRequestContext, dto: CreateAppointmentDto) {
    const start = parseInstant(dto.start);
    const end = parseInstant(dto.end);
    ...
    const value = await this.lock.run(c.workspaceId, dto.staffId, () =>
      this.transactions.run(async (session) => {
        ...
        const [rule, service] = await Promise.all([
          this.availability.findForStaff(c.workspaceId, dto.staffId, session),
          this.services.getActive(c.workspaceId, dto.serviceId, session),
        ]);
        ...
        if (
          ...
          throw new ConflictException('Staff member is unavailable for this time range');
  ...
  async reschedule(c: WorkspaceRequestContext, id: string, dto: RescheduleAppointmentDto) {
    ...
    const value = await this.lock.run(c.workspaceId, String(currentForLock.staffId), () =>
      this.transactions.run(async (session) => {
        const current = await this.repository.getActive(c.workspaceId, id, session);
        const rule = await this.availability.findForStaff(
          c.workspaceId,
  ...
  async cancel(c: WorkspaceRequestContext, id: string, dto: CancelAppointmentDto) {
    const value = await this.repository.updateEntity(c.workspaceId, id, c.userId, dto.version, {
      status: 'cancelled',
