...
import { CrmListQueryDto } from '../../crm/crm.dto.js';
...
export class AppointmentsController {
  ...
  @Get() @RequirePermissions('appointments.read') list(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Query() q: CrmListQueryDto,
  ) {
    return this.service.list(c, q);
  }
  @Get(':id') @RequirePermissions('appointments.read') get(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
  ) {
    return this.service.get(c, id);
  }
  @Post() @RequirePermissions('appointments.manage') create(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Body() d: CreateAppointmentDto,
  ) {
    return this.service.create(c, d);
  }
  @Patch(':id/reschedule') @RequirePermissions('appointments.manage') reschedule(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
    @Body() d: RescheduleAppointmentDto,
  ) {
    return this.service.reschedule(c, id, d);
  }
  @Post(':id/cancel') @RequirePermissions('appointments.manage') cancel(
    @WorkspaceContext() c: WorkspaceRequestContext,
    @Param('id', ParseMongoIdPipe) id: string,
    @Body() d: CancelAppointmentDto,
  ) {
    return this.service.cancel(c, id, d);
  }
}
