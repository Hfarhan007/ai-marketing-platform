...
interface DeliveryJob { workspaceId:string; messageId:string; conversationId:string }
@Injectable()
...
export class OutboundMessageProcessor extends WorkerHost {
  ...
  constructor(private readonly inbox:InboxService){super()}
  async process(job:Job<DeliveryJob>):Promise<void>{
    ...
    try {
      ...
      this.logger.debug({messageId:job.data.messageId,conversationId:job.data.conversationId},'Outbound delivery command accepted');
    } catch(error:unknown) {
      await this.inbox.delivery(job.data.workspaceId,job.data.messageId,{state:'failed',failureCode:'provider_delivery_failed'});
      throw error;
    }
  }
}
