...
export class InboxGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      const workspaceId =
        typeof client.handshake.auth.workspaceId === 'string'
          ? client.handshake.auth.workspaceId
          : '';
      const claims = this.tokens.verify(token);
      const membership = await this.memberships.findActiveMembership(workspaceId, claims.sub);
      if (!membership) throw new WsException('Workspace access denied');
      ...
      this.server
        ...
        .emit('presence.updated', { userId: claims.sub, state: 'online' });
    } catch {
  ...
  handleDisconnect(client: Socket) {
    ...
    if (data.workspaceId && data.userId)
      this.server
        ...
        .emit('presence.updated', { userId: data.userId, state: 'offline' });
  }
  @SubscribeMessage('conversation.join') async join(
    @ConnectedSocket() client: Socket,
    @MessageBody() payload: JoinPayload,
  ) {
    const data = this.auth(client);
    await this.repository.conversation(data.workspaceId, payload.conversationId);
    await client.join(this.room(data.workspaceId, payload.conversationId));
    return { joined: true };
  }
  @SubscribeMessage('conversation.leave') async leave(
    @ConnectedSocket() client: Socket,
    @MessageBody() payload: JoinPayload,
  ) {
    const data = this.auth(client);
    await client.leave(this.room(data.workspaceId, payload.conversationId));
    return { left: true };
  }
  @SubscribeMessage('typing') async typing(
    @ConnectedSocket() client: Socket,
    @MessageBody() payload: TypingPayload,
  ) {
    const data = this.auth(client);
    await this.repository.conversation(data.workspaceId, payload.conversationId);
    client.to(this.room(data.workspaceId, payload.conversationId)).emit('typing.updated', {
  ...
  private auth(client: Socket): { workspaceId: string; userId: string } {
    const data = client.data as SocketData;
    if (!data.workspaceId || !data.userId) throw new WsException('Unauthenticated');
    return { workspaceId: data.workspaceId, userId: data.userId };
  }
  private room(workspaceId: string, conversationId: string) {
