...
export abstract class CrmRepository<T extends CrmEntity> extends TenantAwareRepository<T> {
  ...
  async page(workspaceId: string, query: CrmListQueryDto, filter: mongo.Filter<T> = {}): Promise<CrmPage<T>> {
    const where = {
      ...filter,
    ...
    const [items, total] = await Promise.all([
      this.model.find(where).sort(sort).skip((query.page - 1) * query.limit).limit(query.limit).lean<T[]>().exec(),
  ...
  async getActive(workspaceId: string, id: string, session?: ClientSession): Promise<T> {
    const value = await this.model.findOne({
      ...
      workspaceId: new Types.ObjectId(workspaceId),
      deletedAt: null,
    } as mongo.Filter<T>).session(session ?? null).lean<T>().exec();
    if (!value) throw new NotFoundException('CRM resource not found');
    return value;
  }

  async createEntity(workspaceId: string, actorId: string, input: object, session?: ClientSession): Promise<T> {
  ...
  async updateEntity(
    ...
    const value = await this.model.findOneAndUpdate({
      ...
      $inc: { version: 1 },
    }, { new: true, runValidators: true, session: session ?? null }).lean<T>().exec();
  ...
  async restore(workspaceId: string, id: string, actorId: string, version: number): Promise<T> {
    const value = await this.model.findOneAndUpdate({
      ...
      $inc: { version: 1 },
    }, { new: true, runValidators: true }).lean<T>().exec();
