...
export abstract class CrmCrudService<T extends CrmEntity, C extends object, U extends C & { version: number }> {
  protected constructor(
    ...
    protected readonly entityType: 'companies' | 'leads' | 'deals' | 'pipelines' | 'tasks' | 'services' | 'availability' | 'booking-links',
  ...
  async get(context: WorkspaceRequestContext, id: string) { return this.map(await this.repository.getActive(context.workspaceId, id)); }
  async create(context: WorkspaceRequestContext, dto: C) {
    const value = await this.repository.createEntity(context.workspaceId, context.userId, this.prepare(dto));
  ...
  async update(context: WorkspaceRequestContext, id: string, dto: U) {
    ...
    const value = await this.repository.updateEntity(context.workspaceId, id, context.userId, version, this.prepare(input as C));
  ...
  protected prepare(dto: C): object { return dto; }
  protected record(context: WorkspaceRequestContext, value: T, action: string) {
    return this.events.record({ workspaceId: context.workspaceId, actorId: context.userId, entityType: this.entityType.slice(0, -1), entityId: String(value._id), action });
  }
}
