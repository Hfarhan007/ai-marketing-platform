...
import {
  ...
  WorkflowDeduplicationKeySchema,
  WorkflowDefinition,
...
} from './services/workflow-scheduler.service.js';
...
@Module({
  imports: [
    ...
    CrmModule,
    EventsModule,
    BullModule.registerQueue({ name: WORKFLOW_QUEUE }, { name: WORKFLOW_SCHEDULER_QUEUE }),
    MongooseModule.forFeature([
      ...
      { name: WorkflowStepRun.name, schema: WorkflowStepRunSchema },
      { name: WorkflowWaitState.name, schema: WorkflowWaitStateSchema },
      { name: WorkflowDeduplicationKey.name, schema: WorkflowDeduplicationKeySchema },
    ]),
  ],
