...
import { Agent, AgentMessage, AgentRun, AgentUsage, AgentVersion, ToolExecution } from '../schemas/agent.schemas.js';
...
export class AgentRunsRepository {
  ...
  async createRun(input: { workspaceId: string; agentId: string; agentVersionId: string; requestId: string; correlationId: string; userId: string; deadline: Date }) {
  ...
  async reserveTool(workspaceId: string, runId: string, toolName: string, idempotencyKey: string, sensitive: boolean) {
    ...
    try {
      const execution = await new this.tools({ workspaceId: new Types.ObjectId(workspaceId), runId: new Types.ObjectId(runId), toolName, idempotencyKey, status: sensitive ? 'pending_approval' : 'running' }).save();
  ...
  approveTool(workspaceId: string, executionId: string, approvalId: string) {
    if (!Types.ObjectId.isValid(approvalId)) return Promise.resolve(null);
    return this.tools.findOneAndUpdate({ _id: new Types.ObjectId(executionId), workspaceId: new Types.ObjectId(workspaceId), status: 'pending_approval' }, { $set: { status: 'running', approvedBy: new Types.ObjectId(approvalId) } }, { new: true }).lean().exec();
  }
