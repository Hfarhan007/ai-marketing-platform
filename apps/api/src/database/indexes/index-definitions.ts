...
export const INDEX_DEFINITIONS: readonly ExplicitIndexDefinition[] = [
  ...
  workspaceIndex(
    'knowledge_documents',
    { sourceId: 1, contentHash: 1 },
    'knowledge_document_content_unique',
    { unique: true },
  ),
  workspaceIndex(
    'knowledge_chunks',
    { documentId: 1, ordinal: 1 },
    'knowledge_chunk_ordinal_unique',
    { unique: true },
  ),
  ...
  workspaceIndex(
    ...
    { idempotencyKey: 1 },
    'knowledge_ingestion_job_idempotency',
    { unique: true },
  ),
  workspaceIndex(
    'knowledge_ingestion_jobs',
    { status: 1, updatedAt: 1 },
    'knowledge_ingestion_job_recovery',
  ),
  {
    collection: 'knowledge_vector_index_versions',
    name: 'vector_index_environment_version_unique',
    keys: { environment: 1, collection: 1, version: 1 },
    options: { unique: true },
  },
  {
    collection: 'knowledge_vector_index_versions',
    name: 'vector_index_environment_status',
    keys: { environment: 1, collection: 1, status: 1 },
  },
  workspaceIndex(
    'knowledge_retrieval_logs',
    { createdAt: -1 },
    'knowledge_retrieval_workspace_time',
  ),
  workspaceIndex(
    'knowledge_rag_evaluations',
    { correlationId: 1 },
    'knowledge_evaluation_correlation',
  ),
  workspaceIndex(
    'knowledge_sources',
    { status: 1, createdAt: 1 },
  ...
  workspaceIndex('data_lifecycle_policies', { dataClass: 1 }, 'lifecycle_policy_workspace_class', {
    unique: true,
  }),
  workspaceIndex(
    'data_legal_holds',
  ...
  workspaceIndex('ai_prompt_templates', { key: 1 }, 'ai_prompt_template_workspace_key', {
    unique: true,
  }),
  workspaceIndex(
    'ai_prompt_versions',
  ...
  workspaceIndex(
    ...
    { templateId: 1, feature: 1, environment: 1 },
    'ai_prompt_assignment_unique',
    { unique: true },
  ),
  workspaceIndex(
    'ai_prompt_approval_audits',
    { templateId: 1, version: 1, createdAt: -1 },
    'ai_prompt_approval_history',
  ),
  {
  ...
  workspaceIndex('ai_usage_records', { createdAt: -1 }, 'ai_usage_workspace_created'),
  {
    collection: 'ai_safety_policies',
    name: 'ai_safety_policy_workspace',
    keys: { workspaceId: 1 },
    options: { unique: true },
  },
  {
    collection: 'ai_execution_traces',
    name: 'ai_trace_request_unique',
    keys: { requestId: 1 },
    options: { unique: true },
  },
  ...
  workspaceIndex('ai_feedback', { requestId: 1, userId: 1, kind: 1 }, 'ai_feedback_unique', {
    unique: true,
  }),
  ...
  workspaceIndex(
    'knowledge_connector_connections',
    { status: 1, updatedAt: -1 },
    'knowledge_connector_connections_workspace_status',
  ),
  workspaceIndex(
    'knowledge_connector_sync_runs',
    { connectionId: 1, idempotencyKey: 1 },
    'knowledge_connector_sync_idempotency',
    { unique: true },
  ),
  workspaceIndex(
    'knowledge_connector_sync_runs',
    { connectionId: 1, createdAt: -1 },
    'knowledge_connector_sync_audit_history',
  ),
  workspaceIndex(
    'knowledge_connector_documents',
    { connectionId: 1, externalId: 1 },
    'knowledge_connector_document_unique',
    { unique: true },
  ),
  workspaceIndex(
    'knowledge_connector_documents',
    { connectionId: 1, status: 1 },
    'knowledge_connector_document_inventory',
  ),
  workspaceIndex(
    'file_text_extractions',
    { fileId: 1, sourceChecksum: 1, toolVersion: 1 },
    'file_text_extraction_idempotency',
    { unique: true },
  ),
] as const;
